...
def page_earthquake():
    ...
    **Mengapa simulasi ini penting bagi Anda?**  
    Saat gempa bumi terjadi, tanah akan bergoyang ke samping. Gedung bertingkat harus dirancang fleksibel tetapi kokoh agar goyangan tersebut tidak meruntuhkan tiang penopang bangunan. Mari uji seberapa besar simpangan yang dapat ditoleransi oleh struktur.
    """)
    ...
    with col2:
        ...
        def plot_building(amplitude_t, t_val=0):
            ...
            denom = y_coords[-1] if y_coords[-1] != 0 else 1.0
            x_coords = amplitude_t * (y_coords / denom)**2
            ...
            for t in t_steps:
                ...
                if abs(amplitude) > 6.0 and material == "Beton Biasa (Tanpa Besi Tulangan)":
                    status_placeholder.error(
                        f"🚨 BAHAYA BESAR: Gedung runtuh total pada detik ke-{t:.2f}! "
                        f"Beton biasa langsung pecah karena tidak mampu menahan gaya tarik saat bergoyang "
                        f"(Simpangan = {abs(amplitude):.2f} meter)."
                    )
                    break
                elif abs(amplitude) > 4.0:
                    status_placeholder.warning(
                        f"⚠️ KERUSAKAN STRUKTUR: Tiang beton mengalami retak-retak parah pada detik ke-{t:.2f} "
                        f"(Simpangan = {abs(amplitude):.2f} meter). Perlu inspeksi dan perbaikan struktural."
                    )
                else:
                    status_placeholder.success(
                        f"✅ GEDUNG AMAN: Struktur bergoyang secara aman dan getaran diredam dengan cepat "
                        f"(Simpangan aman = {abs(amplitude):.2f} meter)."
                    )
                time.sleep(0.04)